...
async def test_list_users(test_session: AsyncSession) -> None:
    ...
    await create_user(
        test_session,
        {"email": "user1@example.com", "username": "user1", "hashed_password": "hash1"},
    )
    await create_user(
        test_session,
        {"email": "user2@example.com", "username": "user2", "hashed_password": "hash2"},
    )
    await create_user(
        test_session,
        {"email": "user3@example.com", "username": "user3", "hashed_password": "hash3"},
    )
...
async def test_list_users_pagination(test_session: AsyncSession) -> None:
    ...
    # Get first 2 users
    filters_page1 = UserListFilter(skip=0, limit=2)
    users_page1 = await list_users(test_session, filters_page1)
    assert len(users_page1) == 2

    # Get next 2 users
    filters_page2 = UserListFilter(skip=2, limit=2)
    users_page2 = await list_users(test_session, filters_page2)
...
async def test_list_users_active_only(test_session: AsyncSession) -> None:
    ...
    active_user = await create_user(
        test_session,
        {"email": "active@example.com", "username": "active", "hashed_password": "hash"},
    )

    # Create inactive user
    inactive_user = await create_user(
        test_session,
        {"email": "inactive@example.com", "username": "inactive", "hashed_password": "hash"},
    )
    await update_user(test_session, inactive_user, {"is_active": False})

    # List only active users
    filters = UserListFilter(active_only=True)
    users = await list_users(test_session, filters)
...
async def test_update_user(test_session: AsyncSession) -> None:
    ...
    updated_user = await update_user(
        test_session,
        user,
        {"email": "newemail@example.com", "full_name": "New Name"},
    )
