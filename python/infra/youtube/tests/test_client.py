...
from python.infra.youtube import (
...
from python.infra.youtube.models import VideoInfo
...
def test_get_video_info_invalid_url() -> None:
    ...
    with patch("yt_dlp.YoutubeDL") as mock_ydl_class:
        mock_ydl = MagicMock()
        mock_ydl.extract_info.side_effect = yt_dlp.utils.DownloadError("ERROR: Invalid URL")
        mock_ydl.__enter__ = Mock(return_value=mock_ydl)
        mock_ydl.__exit__ = Mock(return_value=False)
        mock_ydl_class.return_value = mock_ydl
...
def test_download_video_with_format(mock_video_info: dict[str, Any], tmp_path: Path) -> None:
    ...
    with patch("yt_dlp.YoutubeDL") as mock_ydl_class:
        ...
        info = download_video(url, output_path, options)
...
def test_download_audio_success(mock_video_info: dict[str, Any], tmp_path: Path) -> None:
    ...
    with patch("yt_dlp.YoutubeDL") as mock_ydl_class:
        ...
        info = download_audio(url, output_path, options)
...
def test_get_transcriptions_success(mock_video_info_with_subs: dict[str, Any]) -> None:
    ...
    with patch("yt_dlp.YoutubeDL") as mock_ydl_class:
        ...
        en_transcript = next(t for t in manual if t.language == "en")
        assert en_transcript.text, "Transcript text should not be empty"
        assert len(en_transcript.text) > 100, "Transcript should have substantial content"

        # Check for expected content from "Me at the zoo" video
        text_lower = en_transcript.text.lower()
        assert "elephant" in text_lower, "Transcript should mention elephants"
        assert "trunk" in text_lower, "Transcript should mention trunks"
...
def test_get_transcriptions_with_language_filter(mock_video_info_with_subs: dict[str, Any]) -> None:
    ...
    with patch("yt_dlp.YoutubeDL") as mock_ydl_class:
        ...
        for transcript in transcriptions:
            assert transcript.text, f"Transcript for {transcript.language} should have text"
